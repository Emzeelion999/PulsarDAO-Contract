...
contract vePUL is Ownable, Multicall, ReentrancyGuard, ERC721Enumerable, IERC721Receiver {
    ...
    constructor(address tokenAddr, RewardInfo memory _rewardInfo) ERC721("Pulsar DAO veNFT", "vePUL") {
        token = tokenAddr;
    ...
    /// @return weight
    function nftVePUL(uint256 nftId, uint256 timestamp) public view returns(uint256) {
    ...
    /// @param timestamp specified timestamp after latest update of this nft (amount change or end change)
    /// @return weight
    function nftVePULAt(uint256 nftId, uint256 timestamp) public view returns(uint256) {
    ...
    function _totalVePULAt(Point memory point, uint256 timestamp) internal view returns(uint256) {
    ...
    function _updateStakingStatus(uint256 nftId) internal {
        ...
        t.lastVePUL = t.lockAmount / MAXTIME * (Math.max(block.timestamp, nftLocked[nftId].end) - block.timestamp);
    }
    ...
    function stake(uint256 nftId) external nonReentrant {
        ...
        stakingStatus[nftId] = StakingStatus({
            ...
            lastVePUL: lockAmount / MAXTIME * (Math.max(block.timestamp, nftLocked[nftId].end) - block.timestamp),
