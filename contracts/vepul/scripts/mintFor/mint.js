...
// Example: HARDHAT_NETWORK='pulsarTest' node mint.js ${PATH_TO_INPUT_LIST} ${PATH_TO_OUTPUTR_LIST} 18
console.log(v);
console.log(v[4]);
const para = {
    inputPath: v[2],
...
function getTransferCalling(vePUL, nftId, fromAddress, toAddress) {
    return vePUL.methods.safeTransferFrom(fromAddress, toAddress)
}
//mint uniswap v3 nft
async function main() {
    ...
    for (let mintListStart = originSendAddrNum; mintListStart < mintListLen; mintListStart += mintDelta) {
        ...
        for (let idx = 0; idx < mintSubList.length; idx ++) {
            ...
            const mintCalling = getMintCalling(vePUL, amount, endTime);
            mintCallings.push(mintCalling);
        }
        console.log('mintCallings: ', mintCallings);

        const txData = vePUL.methods.multicall(mintCallings).encodeABI()
        const gas = await vePUL.methods.multicall(mintCallings).estimateGas({from: deployer.address});
