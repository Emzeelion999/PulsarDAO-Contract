...
def commit_new_parameters(
    ...
    Pulsar(_pool).commit_new_parameters(
        _new_mid_fee,
        _new_out_fee,
        _new_admin_fee,
        _new_fee_gamma,
...
def ramp_A_gamma(_pool: address, _future_A: uint256, _future_gamma: uint256, _future_time: uint256):
    ...
    """
    assert msg.sender == self.parameter_admin, "Access denied"
    Pulsar(_pool).ramp_A_gamma(_future_A, _future_gamma, _future_time)
...
def stop_ramp_A_gamma(_pool: address):
    ...
    Pulsar(_pool).stop_ramp_A_gamma()
...
def set_aave_referral(_pool: address, referral_code: uint256):
    ...
    """
    assert msg.sender == self.ownership_admin, "Access denied"
    Pulsar(_pool).set_aave_referral(referral_code)  # dev: if implemented by the pool
