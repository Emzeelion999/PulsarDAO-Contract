...
// @title Voting-Escrow PUL (vePUL)
// @author Pulsar
// @license MIT
// @notice Voting-Escrow to have time-weighted votes，
    ...
    IERC721Receiver
{
    ...
    /// @notice weight-curve(vePUL amount) of total-weight for all nft
    ...
    /// @notice total amount of staked PUL
    uint256 public stakePULAmount = 0;
    ...
    constructor(address tokenAddr, RewardInfo memory _rewardInfo)
    ...
    function _findTimestampEpoch(uint256 _timestamp, uint256 maxEpoch)
        internal
        view
        returns (uint256)
    {
    ...
    function _findNftTimestampEpoch(uint256 nftId, uint256 _timestamp)
        internal
        view
        returns (uint256)
    {
    ...
    /// @notice weight of nft (vePUL amount) at certain time after latest update of that nft
    /// @param nftId id of nft
    /// @param timestamp specified timestamp after latest update of this nft (amount change or end change)
    /// @return weight
    function nftVePUL(uint256 nftId, uint256 timestamp)
        public
        view
        returns (uint256)
    {
        ...
        if (_epoch == 0) {
            ...
            lastPoint.bias -=
                lastPoint.slope *
                int256(timestamp - lastPoint.timestamp);
            if (lastPoint.bias < 0) {
                lastPoint.bias = 0;
            }
    ...
    /// @param timestamp specified timestamp after latest update of this nft (amount change or end change)
    /// @return weight
    function nftVePULAt(uint256 nftId, uint256 timestamp)
        public
        view
        returns (uint256)
    {
        ...
        uPoint.bias -=
            uPoint.slope *
            (int256(timestamp) - int256(uPoint.timestamp));
        if (uPoint.bias < 0) {
            uPoint.bias = 0;
        }
    ...
    function _totalVePULAt(Point memory point, uint256 timestamp)
        internal
        view
        returns (uint256)
    {
    ...
    /// @return total weight
    function totalVePUL(uint256 timestamp) external view returns (uint256) {
        uint256 _epoch = epoch;
        Point memory lastPoint = pointHistory[_epoch];
        require(timestamp >= lastPoint.timestamp, "Too Early");
        return _totalVePULAt(lastPoint, timestamp);
    }

    /// @notice total weight of all nft at a certain time
    /// @param timestamp specified blockNumber, "certain time" in above line
    /// @return total weight
    function totalVePULAt(uint256 timestamp) external view returns (uint256) {
        uint256 _epoch = epoch;
        ...
        if (targetEpoch == _epoch) {
            return _totalVePULAt(point, timestamp);
        } else {
            point.bias =
                point.bias -
                point.slope *
                (int256(timestamp) - int256(point.timestamp));
            if (point.bias < 0) {
                point.bias = 0;
            }
    ...
    function _updateStakingStatus(uint256 nftId) internal {
        ...
        t.lastVePUL =
            ...
            (Math.max(block.timestamp, nftLocked[nftId].end) - block.timestamp);
    }
    ...
    function unStake() external nonReentrant {
        ...
        stakePULAmount -= uint256(nftLocked[nftId].amount);
    ...
    /// @return reward PUL reward amount
        ...
        returns (uint256 reward)
    {
        ...
        StakingStatus memory t = stakingStatus[nftId];
        if (t.stakingId != 0) {
            // we are sure that stakePULAmount is not 0
