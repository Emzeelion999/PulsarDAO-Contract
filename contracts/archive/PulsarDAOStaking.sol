...
contract PulsarDAOStaking is ERC20("vePUL Token", "vePUL"), Ownable {
    ...
    constructor(
        ...
        require(
            address(_pul) != address(0),
            "PulsarDAOStaking: _pul cannot be the zero address"
        );
        pul = _pul;
    ...
    function addRewardPUL(uint256 _pulAmount) external {
        ...
        require(
            block.timestamp < cfg.periodFinish,
            "PulsarDAOStaking: Adding rewards is forbidden"
        );
     ...
     * @param _rewardsDuration the duration of rewards in seconds
     */
    function setPeriod(uint64 _periodStart, uint64 _rewardsDuration)
        public
        onlyOwner
    {
        require(
            _periodStart >= block.timestamp,
            "PulsarDAOStaking: _periodStart shouldn't be in the past"
        );
        require(
            _rewardsDuration > 0,
            "PulsarDAOStaking: Invalid rewards duration"
        );

        Config memory cfg = config;
        require(
            cfg.periodFinish < block.timestamp,
            "PulsarDAOStaking: The last reward period should be finished before setting a new one"
        );
    ...
    function enter(uint256 _pulAmount) external {
        require(
            _pulAmount > 0,
            "PulsarDAOStaking: Should at least stake something"
        );
    ...
    function leave(uint256 _share) external {
        require(
            _share > 0,
            "PulsarDAOStaking: Should at least unstake something"
        );
