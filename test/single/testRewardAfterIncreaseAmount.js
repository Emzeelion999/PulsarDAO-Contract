...
describe("test increase unlock time", function () {
  ...
  it("increase amount", async function () {
    ...
    const expectReward4 = stringDiv(
      stringMul(stringAdd(deltaGlobalAcc3, deltaGlobalAcc4), lastVePUL2),
      q128
    );
    ...
    const deltaGlobalAcc5 = stringDiv(
      stringMul(stringMul(rewardPerBlock, "5"), q128),
      stakePULAmount3
    );

    const expectReward5 = stringDiv(
      stringMul(deltaGlobalAcc5, lastVePUL4),
      q128
    );
    expect(reward4).to.equal(expectReward4);
  });
