...
async function tryIncreaseUnlockTime(vePUL, PUL, nftId, tester, endTime) {
  const PULBalanceBefore = (await PUL.balanceOf(tester.address)).toString();
  await vePUL.connect(tester).increaseUnlockTime(nftId, endTime);
  const PULBalanceAfter = (await PUL.balanceOf(tester.address)).toString();
  return stringMinus(PULBalanceAfter, PULBalanceBefore);
}
...
describe("test increase unlock time", function () {
  ...
  it("increase amount", async function () {
    ...
    expect(reward2).to.equal(
      stringDiv(stringMul(lastVePUL1, deltaGlobalAcc2), q128)
    );
    ...
    const reward3 = await tryIncreaseUnlockTime(
      vePUL,
      PUL,
      "1",
      tester,
    ...
    const expectReward3 = stringDiv(
      stringMul(deltaGlobalAcc3, lastVePUL2),
      q128
    );
    expect(reward3).to.equal(expectReward3);

    const stakingStatus3 = await getStakingStatus(vePUL, "1");
    const lastVePUL3 = stringMul(slope, remainTime3);
    expect(lastVePUL3).to.equal(stakingStatus3.lastVePUL);
    ...
    const deltaGlobalAcc4 = stringDiv(
      stringMul(stringMul(rewardPerBlock, "20"), q128),
      stakePULAmount
    );

    const expectReward4 = stringDiv(
      stringMul(deltaGlobalAcc4, lastVePUL3),
      q128
    );
