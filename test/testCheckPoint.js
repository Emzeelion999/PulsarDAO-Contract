...
async function getNftLocked(vePUL, nftId) {
  const nftLocked = await vePUL.nftLocked(nftId);
  return {
    amount: Number(nftLocked.amount.toString()),
    end: Number(nftLocked.end.toString()),
  };
}

async function getPoint(vePUL, epoch) {
  const point = await vePUL.pointHistory(epoch);
  return {
    bias: point.bias.toString(),
    slope: point.slope.toString(),
...
describe("test uniswap price oracle", function () {
  ...
  beforeEach(async function () {
    ...
    await PUL.connect(tester).approve(
      vePUL.address,
      decimalToUnDecimalStr(100000000)
    );
    await PUL.mint(tester.address, decimalToUnDecimalStr(100000000));
  });
  ...
  it("check point", async function () {
    ...
    let PULBalance = (await PUL.balanceOf(tester.address)).toString();
    ...
    const PULAmount1 = decimalToUnDecimalStr(10);
    await vePUL.connect(tester).createLock(PULAmount1, unlockTime1);

    const segment1 = getBiasAndSlope(
      PULAmount1,
      unlockTime1 - startTime1,
      MAXTIME
    );
    console.log("segment1: ", segment1);
    const point1 = await getPoint(vePUL, 1);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount1);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    const PULAmount2 = decimalToUnDecimalStr(5);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime2]);
    await vePUL.connect(tester).createLock(PULAmount2, unlockTime2);
    const segment2 = getBiasAndSlope(
      PULAmount2,
      unlockTime2 - startTime2,
      MAXTIME
    );
    // compute current point
    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime2 - startTime1), currentPoint.slope)
    );
    ...
    const point2 = await getPoint(vePUL, 2);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount2);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    const PULAmount3 = decimalToUnDecimalStr(7);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime3]);
    await vePUL.connect(tester).createLock(PULAmount3, unlockTime3);
    const segment3 = getBiasAndSlope(
      PULAmount3,
      unlockTime3 - startTime3,
      MAXTIME
    );
    // compute current point
    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime3 - startTime2), currentPoint.slope)
    );
    ...
    const point3 = await getPoint(vePUL, 3);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount3);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    const PULAmount4 = decimalToUnDecimalStr(30);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime4]);
    await vePUL.connect(tester).createLock(PULAmount4, unlockTime4);
    const segment4 = getBiasAndSlope(
      PULAmount4,
      unlockTime4 - startTime4,
      MAXTIME
    );

    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime4 - startTime3), currentPoint.slope)
    );
    ...
    const point4 = await getPoint(vePUL, 4);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount4);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    const PULAmount5 = decimalToUnDecimalStr(23);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime5]);
    await vePUL.connect(tester).createLock(PULAmount5, unlockTime5);
    const segment5 = getBiasAndSlope(
      PULAmount5,
      unlockTime5 - startTime5,
      MAXTIME
    );

    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime5 - startTime4), currentPoint.slope)
    );
    ...
    const point5 = await getPoint(vePUL, 5);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount5);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    const PULAmount6 = decimalToUnDecimalStr(92);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime6]);
    await vePUL.connect(tester).createLock(PULAmount6, unlockTime6);
    const segment6 = getBiasAndSlope(
      PULAmount6,
      unlockTime6 - startTime6,
      MAXTIME
    );

    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime6 - startTime5), currentPoint.slope)
    );
    ...
    const point6 = await getPoint(vePUL, 6);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount6);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    const PULAmount7 = decimalToUnDecimalStr(18);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime7]);
    await vePUL.connect(tester).createLock(PULAmount7, unlockTime7);
    const segment7 = getBiasAndSlope(
      PULAmount7,
      unlockTime7 - startTime7,
      MAXTIME
    );

    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime7 - startTime6), currentPoint.slope)
    );
    ...
    const point7 = await getPoint(vePUL, 7);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount7);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    const PULAmount8 = decimalToUnDecimalStr(12);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime8]);
    await vePUL.connect(tester).createLock(PULAmount8, unlockTime8);
    const segment8 = getBiasAndSlope(
      PULAmount8,
      unlockTime8 - startTime8,
      MAXTIME
    );

    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime8 - startTime7), currentPoint.slope)
    );
    ...
    const point8 = await getPoint(vePUL, 8);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount8);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    const PULAmount9 = decimalToUnDecimalStr(215);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime9]);
    await vePUL.connect(tester).createLock(PULAmount9, unlockTime9);
    const segment9 = getBiasAndSlope(
      PULAmount9,
      unlockTime9 - startTime9,
      MAXTIME
    );

    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime9 - startTime8), currentPoint.slope)
    );
    ...
    const point9 = await getPoint(vePUL, 9);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount9);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    const PULAmount10 = decimalToUnDecimalStr(11);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime10]);
    await vePUL.connect(tester).createLock(PULAmount10, unlockTime10);
    const segment10 = getBiasAndSlope(
      PULAmount10,
      unlockTime10 - startTime10,
      MAXTIME
    );

    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime10 - startTime9), currentPoint.slope)
    );
    ...
    const point10 = await getPoint(vePUL, 10);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount10);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    const PULAmount11 = decimalToUnDecimalStr(115);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime11]);
    await vePUL.connect(tester).createLock(PULAmount11, unlockTime11);
    const segment11 = getBiasAndSlope(
      PULAmount11,
      unlockTime11 - startTime11,
      MAXTIME
    );

    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime11 - startTime10), currentPoint.slope)
    );
    ...
    const point11 = await getPoint(vePUL, 11);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount11);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(checkPoint - startTime11), currentPoint.slope)
    );
    ...
    // await vePUL.connect(tester).checkPoint();

    // const point12 = await getPoint(vePUL, 12);
    ...
    const PULAmount12 = decimalToUnDecimalStr(51);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime12]);
    await vePUL.connect(tester).createLock(PULAmount12, unlockTime12);
    const segment12 = getBiasAndSlope(
      PULAmount12,
      unlockTime12 - startTime12,
      MAXTIME
    );

    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime12 - checkPoint), currentPoint.slope)
    );
    ...
    checkPointEpoch += 1;
    const point12 = await getPoint(vePUL, checkPointEpoch);
    expect(point12.bias).to.equal(currentPoint.bias);
    expect(point12.slope).to.equal(currentPoint.slope);
    expect(point12.timestamp).to.equal(startTime12);
    PULBalance = stringMinus(PULBalance, PULAmount12);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    const PULAmount13 = decimalToUnDecimalStr(16);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime13]);
    await vePUL.connect(tester).createLock(PULAmount13, unlockTime13);
    const segment13 = getBiasAndSlope(
      PULAmount13,
      unlockTime13 - startTime13,
      MAXTIME
    );

    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime13 - startTime12), currentPoint.slope)
    );
    ...
    checkPointEpoch += 1;
    const point13 = await getPoint(vePUL, checkPointEpoch);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount13);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    const PULAmount14 = decimalToUnDecimalStr(6);
    await ethers.provider.send("evm_setNextBlockTimestamp", [startTime14]);
    await vePUL.connect(tester).createLock(PULAmount14, unlockTime14);
    const segment14 = getBiasAndSlope(
      PULAmount14,
      unlockTime14 - startTime14,
      MAXTIME
    );

    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(startTime14 - startTime13), currentPoint.slope)
    );
    ...
    checkPointEpoch += 1;
    const point14 = await getPoint(vePUL, checkPointEpoch);
    ...
    PULBalance = stringMinus(PULBalance, PULAmount14);
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      PULBalance
    );
    ...
    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(checkPoint20 - startTime14), currentPoint.slope)
    );
    ...
    // const point15 = await getPoint(vePUL, checkPointEpoch);
    ...
    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(checkPoint25 - checkPoint20), currentPoint.slope)
    );
    ...
    // const pointAt25 = await getPoint(vePUL, checkPointEpoch);
    ...
    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(checkPoint30 - checkPoint25), currentPoint.slope)
    );
    ...
    currentPoint.bias = stringMinus(
      currentPoint.bias,
      stringMul(String(checkPoint32 - checkPoint30), currentPoint.slope)
    );
    ...
    checkPointEpoch += 1;
    const pointAt32 = await getPoint(vePUL, checkPointEpoch);
