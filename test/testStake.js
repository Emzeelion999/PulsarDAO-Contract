...
describe("test increase unlock time", function () {
  ...
  it("stake and collect", async function () {
    ...
    expect((await vePUL.stakedNftOwners("1")).toLowerCase()).to.equal(
    ...
    const deltaGlobalAcc2 = stringDiv(
      stringMul(stringMul(rewardPerBlock, "15"), q128),
      stakePULAmount_1
    );
    ...
    expect((await vePUL.stakedNftOwners("2")).toLowerCase()).to.equal(
    ...
    const deltaGlobalAcc3 = stringDiv(
      stringMul(stringMul(rewardPerBlock, "10"), q128),
      stakePULAmount_2
    );
    const globalAcc3 = stringAdd(globalAcc2, deltaGlobalAcc3);
    expect(PULReward3).to.equal(
      stringDiv(stringMul(lastVePUL1_1, globalAcc3), q128)
    );
    ...
    const deltaGlobalAcc4 = stringDiv(
      stringMul(stringMul(rewardPerBlock, "2"), q128),
      stakePULAmount_3
    );
    ...
    expect((await vePUL.stakedNftOwners("7")).toLowerCase()).to.equal(
      other2.address.toLowerCase()
    );
    ...
    const deltaGlobalAcc5 = stringDiv(
      stringMul(stringMul(rewardPerBlock, "3"), q128),
      stakePULAmount_4
    );
    const globalAcc5 = stringAdd(globalAcc4, deltaGlobalAcc5);
    const expectReward2_5 = stringDiv(
      stringMul(stringMinus(globalAcc5, globalAcc2), lastVePUL2_2),
      q128
    );
    ...
    expect(
      BigNumber((await vePUL.stakedNftOwners("2")).toLowerCase()).toFixed(0)
    ).to.equal("0");
    ...
    const deltaGlobalAcc6 = stringDiv(
      stringMul(stringMul(rewardPerBlock, "6"), q128),
      stakePULAmount_5
    );
    const globalAcc6 = stringAdd(globalAcc5, deltaGlobalAcc6);
    expect(PULReward6).to.equal(
      stringDiv(
        stringMul(lastVePUL1_3, stringMinus(globalAcc6, globalAcc3)),
        q128
      )
    );
    ...
    const deltaGlobalAcc7 = stringDiv(
      stringMul(stringMul(rewardPerBlock, "1"), q128),
      stakePULAmount_6
    );
    const globalAcc7 = stringAdd(globalAcc6, deltaGlobalAcc7);
    const expectReward1_7 = stringDiv(
      stringMul(stringMinus(globalAcc7, globalAcc6), lastVePUL1_6),
      q128
    );
    ...
    expect(
      BigNumber((await vePUL.stakedNftOwners("1")).toLowerCase()).toFixed(0)
    ).to.equal("0");
  });
});
