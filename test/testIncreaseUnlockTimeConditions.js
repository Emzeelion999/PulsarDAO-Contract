...
describe("test increase unlock time", function () {
  ...
  beforeEach(async function () {
    ...
    await vePUL
      .connect(tester)
      .createLock("100000000", timestampStart + WEEK * 21.2);
    await vePUL
      .connect(other)
      .createLock("200000000", timestampStart + WEEK * 30);
  });
  ...
  it("increase lock of tester, unstaked", async function () {
    ...
    try {
      await vePUL
        .connect(tester)
        .increaseUnlockTime("1", timestampStart + WEEK * 31.3);
    } catch (err) {
      // console.log(err);
      ok = false;
    }
  ...
  it("increase lock of tester, staked", async function () {
    ...
    try {
      await vePUL
        .connect(tester)
        .increaseUnlockTime("1", timestampStart + WEEK * 31);
    } catch (err) {
      // console.log(err);
      ok = false;
    }
  ...
  it("increase lock of other, unstaked", async function () {
    ...
    try {
      await vePUL
        .connect(tester)
        .increaseUnlockTime("2", timestampStart + WEEK * 35);
    } catch (err) {
      // console.log(err);
      ok = false;
    }
  ...
  it("try to make endtime of lock earlier than before", async function () {
    ...
    try {
      await vePUL
        .connect(tester)
        .increaseUnlockTime("1", timestampStart + WEEK * 15);
    } catch (err) {
      // console.log(err);
      ok = false;
    }
  ...
  it("increase an expired lock to time before current block.timestamp", async function () {
    ...
    try {
      ...
      await vePUL
        .connect(tester)
        .increaseUnlockTime("1", timestampStart + WEEK * 25);
    } catch (err) {
      // console.log(err);
      ok = false;
    }
  ...
  it("try to set endtime later than 4 years", async function () {
    ...
    try {
      ...
      await vePUL
        ...
        .increaseUnlockTime(
          "1",
          timestampStart + Math.round(WEEK * 25.5) + MAXTIME + 1 + WEEK
        );
    } catch (err) {
      // console.log(err);
      ok = false;
