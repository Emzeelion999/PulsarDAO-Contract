...
function getIncreaseAmountCalling(nftId, amount, vePULAddress) {
  const vePUL = new web3.eth.Contract(abi, vePULAddress);
  return vePUL.methods.increaseAmount(nftId, amount).encodeABI();
}
...
describe("test increase amount", function () {
  ...
  it("at 20 WEEK, create a new lock, increase amount of 3", async function () {
    ...
    const callings = [
      ...
      getIncreaseAmountCalling(nftId, amount, vePUL.address),
    ];
    ...
    locks[nftId - 1].slope = Number(
      stringDiv(String(locks[nftId - 1].amount), MAXTIME)
    );
    locks[nftId - 1].bias =
      locks[nftId - 1].slope *
      (locks[nftId - 1].endTime - locks[nftId - 1].startTime);

    const lock1 = await vePUL.nftLocked(nftId);
