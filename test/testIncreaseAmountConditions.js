...
describe("test increase unlock time", function () {
  ...
  beforeEach(async function () {
    ...
    await vePUL
      .connect(tester)
      .createLock("10000000000", timestampStart + WEEK * 21.2);
    await vePUL
      .connect(other)
      .createLock("20000000000", timestampStart + WEEK * 30);
  ...
  it("increase Amount of tester, unstaked", async function () {
    ...
    expect((await PUL.balanceOf(tester.address)).toString()).to.equal(
      stringMinus(balance, "5000000000")
    );
  });
  ...
  it("increase Amount of other, staked", async function () {
    ...
    const acc = stringDiv(stringMul(rewardPerBlock, q128), stakePULAmount);
    const reward = stringDiv(
      stringMul(acc, stakingStatus2.lastVePUL.toString()),
      q128
    );
