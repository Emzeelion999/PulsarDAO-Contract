...
function getIncreaseUnlockTimeCalling(nftId, endTime, vePULAddress) {
  const vePUL = new web3.eth.Contract(abi, vePULAddress);
  return vePUL.methods.increaseUnlockTime(nftId, endTime).encodeABI();
}
...
describe("test increase unlock time", function () {
  ...
  it("at 8.1 WEEK, increase a lock to 23 WEEK", async function () {
    ...
    await vePUL
      .connect(tester)
      .increaseUnlockTime(7, timestampStart + 23 * WEEK);
  ...
  it("at 16 WEEK, increase a lock(nftId:1) to 30 WEEK", async function () {
    ...
    await vePUL
      .connect(tester)
      .increaseUnlockTime(1, timestampStart + 30 * WEEK);
  ...
  it("at 16.5 WEEK, increase a lock(nftId:7) to 26.5(actually 26) WEEK", async function () {
    ...
    await vePUL
      .connect(tester)
      .increaseUnlockTime(7, timestampStart + 26 * WEEK + Math.round(WEEK / 2));
  ...
  it("at 19.5 WEEK, increase a lock(nftId:7) to 26.2(actualy 26) WEEK", async function () {
    ...
    await vePUL
      ...
      .increaseUnlockTime(
        7,
        timestampStart + 26 * WEEK + Math.round(0.2 * WEEK)
      );
