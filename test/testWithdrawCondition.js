...
describe("test withdraw condition", function () {
    ...
    beforeEach(async function() {
        ...
        await vePUL.connect(tester).createLock('10000000000', timestampStart + WEEK * 10);
        await vePUL.connect(other).createLock('20000000000', timestampStart + WEEK * 10);
    ...
    it("withdraw at week 10, unstaked", async function () {
        ...
        expect((await PUL.balanceOf(tester.address)).toString()).to.equal(stringAdd(balance, '10000000000'));
    });
    ...
    it("withdraw others, at 20 week", async function () {
        ...
        let ok = true;
        try {
            await vePUL.connect(tester).withdraw('2');
        } catch(err) {
            // console.log(err);
            ok = false;
        }
