...
function getNewLockCalling(amount, endTime, vePULAddress) {
  const vePUL = new web3.eth.Contract(abi, vePULAddress);
  return vePUL.methods.createLock(amount, endTime).encodeABI();
}

function getMergeCalling(nftFrom, nftTo, vePULAddress) {
  const vePUL = new web3.eth.Contract(abi, vePULAddress);
  return vePUL.methods.merge(nftFrom, nftTo).encodeABI();
}
...
describe("test increase merge", function () {
  ...
  beforeEach(async function () {
    ...
    await vePUL.connect(tester).multicall(createLockCallings1);
    ...
    await vePUL.connect(tester).multicall(createLockCallings2);
    ...
    await vePUL.connect(tester).multicall(createLockCallings3);
  });
  ...
  it("at 20 WEEK, create a new lock, merge 7 to 2", async function () {
    ...
    const callings = [
      ...
      getMergeCalling(7, 2, vePUL.address),
    ];
    balance = stringMinus(balance, locks[8].amount);
    await vePUL.connect(tester).multicall(callings);
    ...
    const { bias, slope, slopeChanges } = getLastPointAndSlopeChanges(
      locks,
      startTime
    );
    const epoch = await vePUL.epoch();
    ...
    expect(point.timestamp.toString()).to.equal(
      BigNumber(startTime).toFixed(0)
    );

    for (var week = 21; week <= 35; week++) {
      const checkTime1 = timestampStart + week * WEEK - Math.round(WEEK / 2);
      const sc1 = (await vePUL.slopeChanges(checkTime1)).toString();
      expect(sc1).to.equal("0");

      const checkTime2 = timestampStart + week * WEEK;
      const sc2 = (await vePUL.slopeChanges(checkTime2)).toString();
  ...
  it("at 20 WEEK, create a new lock, merge 4 to 1", async function () {
    ...
    const callings = [
      getNewLockCalling(locks[8].amount, locks[8].endTime),
      getMergeCalling(4, 1, vePUL.address),
    ];
    balance = stringMinus(balance, locks[8].amount);
    await vePUL.connect(tester).multicall(callings);
  ...
  it("at 20 WEEK, create a new lock, merge 7 to 6", async function () {
    ...
    const callings = [
      getNewLockCalling(locks[8].amount, locks[8].endTime),
      getMergeCalling(fromId, toId, vePUL.address),
    ];
    balance = stringMinus(balance, locks[8].amount);
    await vePUL.connect(tester).multicall(callings);

    locks[toIdx].amount += locks[fromIdx].amount;
    locks[toIdx].slope = Number(
      stringDiv(String(locks[toIdx].amount), MAXTIME)
    );
    locks[toIdx].bias =
      locks[toIdx].slope * (locks[toIdx].endTime - locks[toIdx].startTime);
  ...
  it("at 18.9 WEEK, merge 8 to 6", async function () {
    ...
    const balance = (await PUL.balanceOf(tester.address)).toString();
    await vePUL.connect(tester).merge(fromId, toId);
