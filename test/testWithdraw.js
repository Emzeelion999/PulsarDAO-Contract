...
function getWithdrawCalling(nftId, amount, vePULAddress) {
  const vePUL = new web3.eth.Contract(abi, vePULAddress);
  return vePUL.methods.withdraw(nftId).encodeABI();
}
...
describe("test increase amount", function () {
  ...
  it("at 20 WEEK, create a new lock, withdraw 1", async function () {
    ...
    const callings = [
      ...
      getWithdrawCalling(nftId, vePUL.address),
    ];
  ...
  it("at 20.5 WEEK withdraw 7", async function () {
    ...
    balance = stringAdd(balance, String(locks[nftId - 1].amount));
    await vePUL.connect(tester).withdraw(nftId);
