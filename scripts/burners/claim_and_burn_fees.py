...
# and "burn" them, converting to 3PUL and sending it to the
# fee distributor to be claimed by vePUL holders. All of the
...
COINS = [
    ...
    "0x075b1bb99792c9E1041bA13afEf80C91a1e70fB3",  # sbtcPUL
    ...
    # meta-burner, for assets that can be directly swapped to 3PUL
...
def main(acct=CALLER, claim_threshold=CLAIM_THRESHOLD):
    ...
    # call burners to convert fee tokens to 3PUL
    burn_start = 0
    to_burn = []
    ...
    # deposits DAI/USDC/USDT into 3pool and transfers the 3PUL to the fee distributor
    ...
    # finally, call to burn 3PUL - this also triggers a token checkpoint
    proxy.burn(lp_tripool, {"from": acct, "gas_price": gas_strategy})

    final = lp_tripool.balanceOf(distributor)
    print(f"Success! Total 3PUL fowarded to distributor: {(final-initial_balance)/1e18:.4f}")
