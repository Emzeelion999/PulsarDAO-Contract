...
def main(network_name, lp_token):
    ...
    streamer = ChildChainStreamer.deploy(
        deployer, claimer, pul, {"from": stream_deployer, "required_confs": CONFS}
    )
    claimer.set_reward_data(0, streamer, pul, {"from": deployer, "required_confs": CONFS})
