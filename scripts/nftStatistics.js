...
async function main() {
  ...
  const nftNum = Number(
    (await vePULContract.methods.nftNum().call()).toString()
  );
  console.log("nftNum: ", nftNum);
  const nftIds = Array(nftNum)
    .fill()
    .map((_, i) => i + 1);
  console.log("nftids: ", nftIds);
  const web3 = getWeb3();
  const nftLocked = await vePULLib.getNftLocked(web3, vePULContract, nftIds);
  const stakedNftOwners = await vePULLib.getStakedNftOwners(
    web3,
    vePULContract,
    nftIds
  );
  const nftOwners = await vePULLib.getNftOwners(web3, vePULContract, nftIds);
  const stakingStatus = await vePULLib.getStakingStatus(
    web3,
    vePULContract,
    nftIds
  );
  ...
  for (let i = 0; i < nftIds.length; i++) {
    ...
    const vePUL = BigNumber(vePULNoDecimal)
      .div(10 ** decimals)
      .toFixed(15);
    const amount = BigNumber(nftLocked[i].amount.toString())
      .div(10 ** decimals)
      .toFixed(15);
    ...
    const nft = {
      ...
      amount,
      owner,
      staking: staking ? "staking" : "unStaking",
    };
    console.log("nft: ", nft);
