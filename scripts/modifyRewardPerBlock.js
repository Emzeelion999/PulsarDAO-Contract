...
const para = {
  miningPoolAddr: contracts[net]["vePUL"],
...
const web3 = getWeb3();
const miningABI = getContractABI(
  __dirname + "/../artifacts/contracts/vePUL.sol/vePUL.json"
);

async function main() {
  para.rewardPerBlockNoDecimal = BigNumber(
    Number(para.rewardPerBlockDecimal) * 10 ** para.rewardTokenDecimal
  ).toFixed(0);
  ...
  const txData = await mining.methods
    .modifyRewardPerBlock(para.rewardPerBlockNoDecimal)
    .encodeABI();
  const gasLimit = await mining.methods
    .modifyRewardPerBlock(para.rewardPerBlockNoDecimal)
