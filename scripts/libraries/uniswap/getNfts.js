  ...
  for (let i = 0; i < nftNum; i += 20) {
    ...
    for (let j = i; j < boundary; j++) {
      tokenIdMulticallData.push(
        nftManager.methods.tokenOfOwnerByIndex(account, j).encodeABI()
      );
    }
    const response = await nftManager.methods
      .multicall(tokenIdMulticallData)
      .call();
    for (nft of response) {
...
async function getNftDetails(web3, nftManager, nftIds) {
  ...
  for (let i = 0; i < nftNum; i += 100) {
    ...
    for (let j = i; j < boundary; j++) {
      tokenIdMulticallData.push(
        nftManager.methods.positions(nftIds[j]).encodeABI()
      );
    }
    const response = await nftManager.methods
      .multicall(tokenIdMulticallData)
