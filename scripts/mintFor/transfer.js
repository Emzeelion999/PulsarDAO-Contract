...
function getTransferCalling(vePUL, fromAddress, toAddress, nftId) {
    return vePUL.methods.safeTransferFrom(fromAddress, toAddress, nftId).encodeABI();
}
//mint uniswap v3 nft
async function main() {
    ...
    for (let mintListStart = originSendAddrNum; mintListStart < mintListLen; mintListStart += mintDelta) {
        ...
        for (let idx = 0; idx < mintSubList.length; idx ++) {
            ...
            const calling = getTransferCalling(vePUL, sender.address, mintSubList[idx].address, nftSubList[idx])
            callings.push(calling);
        }
        console.log('callings: ', callings);

        const txData = vePUL.methods.multicall(callings).encodeABI()
        const gas = await vePUL.methods.multicall(callings).estimateGas({from: sender.address});
